...
export function DeleteDialog({ label, onClose, onDelete }: { label: string; onClose: () => void; onDelete: () => void }) {
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal publish-modal" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><header><div><span className="section-kicker danger-kicker">DELETE</span><h2>Remove this record?</h2></div><button type="button" className="icon-button" onClick={onClose} aria-label="Close delete confirmation"><Icons.close/></button></header><p><b>{label}</b> will be removed from the workbench. The deletion becomes permanent after publishing the next GitHub revision.</p><footer><button type="button" className="button ghost" onClick={onClose}>Keep record</button><button type="button" className="button danger" onClick={onDelete}><Icons.trash/>Delete record</button></footer></div></div>;
}
