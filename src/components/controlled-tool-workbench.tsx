...
export function ControlledToolWorkbench({ workspace, mutate }: { workspace: SkillWorkspace; mutate: Mutate }) {
  ...
  function saveTool() {
    ...
    mutate((current) => {
      ...
      return recordGovernedVersion(next, "controlled_tool", id, prior ? "tool.updated" : "tool.created", "current-user", record as unknown as Record<string, unknown>);
    });
    setDraft(null);
  }
