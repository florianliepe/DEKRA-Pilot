...
export function JobMappingWorkbench({ workspace, secret, mutate, onWorkspace, onMessage, onError }: Props) {
  ...
  return <div className="mapping-workbench">
    ...
    <section className="mapping-main">
      ...
      <article className="panel ai-mapping-command"><div className="agent-orb"><Icons.spark/></div><div><span className="section-kicker">AI SKILL DESIGN AGENT</span><h3>Map this job against approved skills and controlled tools</h3><p>The agent extracts evidence, applies the design standard, scores each match and drafts a profile for human approval. It cannot approve, publish, delete or create licensed content.</p><div className="tool-chips">{["read_catalog", "read_controlled_tools", "find_duplicates", "validate_syntax", "map_job_skills", "create_review_draft"].map((tool) => <span key={tool}>{tool}</span>)}</div></div><button className="button primary" disabled={busy} onClick={() => void analyse()}><Icons.spark/>{busy ? "Agent mapping…" : "Run governed mapping"}</button></article>
      <article className="panel mapping-table"><header><div><span className="section-kicker">SKILL PROFILE PROPOSAL</span><h3>{mappings.length} mapped skills</h3></div><div className={totalWeight === 100 ? "weight-valid" : "weight-warning"}>{totalWeight}% allocated</div></header><div className="mapping-head"><span>Catalog skill</span><span>Evidence & rationale</span><span>Level</span><span>Weight</span><span>Fit</span><span/></div>{mappings.map((mapping) => { const skill = workspace.skills.find((item) => item.id === mapping.skillId); if (!skill) return null; const vectors = mapping.strategicVectorIds.map((id) => workspace.strategicVectors.find((item) => item.id === id)?.name).filter(Boolean); const tools = (mapping.toolIds || []).map((id) => workspace.tools.find((item) => item.id === id)?.name).filter(Boolean); return <div className="mapping-row" key={mapping.id}><span><i className={`dimension-dot ${skill.dimension}`}/><b>{skill.name}</b><small>{skill.dimension} · {mapping.critical ? "critical" : "supporting"}</small>{vectors.length > 0 && <em>{vectors.join(" · ")}</em>}{tools.length > 0 && <em>Tools: {tools.join(" · ")}</em>}</span><span><b>{mapping.rationale}</b><small>{mapping.evidence[0] || "Reviewer evidence required"}</small>{mapping.scoreBreakdown && <em>Evidence {mapping.scoreBreakdown.evidence} · taxonomy {mapping.scoreBreakdown.taxonomy} · level {mapping.scoreBreakdown.proficiency} · strategy {mapping.scoreBreakdown.strategic}</em>}</span><select aria-label={`${skill.name} mapped level`} value={mapping.targetLevel} onChange={(event) => mutate((current) => ({ ...current, mappings: current.mappings.map((item) => item.id === mapping.id ? { ...item, targetLevel: Number(event.target.value) as 1 | 2 | 3 | 4 } : item) }))}>{proficiencyLevels.map((level) => <option value={level.id} key={level.id}>{level.id} · {level.name}</option>)}</select><input aria-label={`${skill.name} mapping weight`} type="number" min="0" max="100" value={mapping.weight} onChange={(event) => mutate((current) => ({ ...current, mappings: current.mappings.map((item) => item.id === mapping.id ? { ...item, weight: Number(event.target.value) } : item) }))}/><span className="fit-score"><b>{mapping.relevance}%</b><small>{mapping.status}</small></span><span className="record-actions"><button aria-label={`Edit ${skill.name} mapping`} onClick={() => setEditingMapping(mapping)}><Icons.edit/></button><button aria-label={`Remove ${skill.name} mapping`} onClick={() => mutate((current) => ({ ...current, mappings: current.mappings.filter((item) => item.id !== mapping.id) }))}><Icons.trash/></button></span></div>; })}<footer><button className="button secondary" onClick={() => setEditingMapping("new")} disabled={mappings.length >= 10}><Icons.plus/>Add catalog skill</button><span>Maximum 10 core skills per pilot role · every agent proposal requires accountable approval</span></footer></article>
...
function emptyMapping(jobDescriptionId: string, workspace: SkillWorkspace, mappings: JobSkillMapping[]): JobSkillMapping {
  ...
  return { id: "", jobDescriptionId, skillId: skill?.id || "", targetLevel: 2, weight: 10, critical: false, relevance: 70, rationale: "", evidence: [], strategicVectorIds: [], toolIds: [], scoreBreakdown: { evidence: 70, taxonomy: 70, proficiency: 70, strategic: 50 }, source: "manual", status: "proposed" };
}
