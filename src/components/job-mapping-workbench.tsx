...
import { analyseMappingProfile, mappingConfidenceInterval, ROLE_SKILL_MAX, ROLE_SKILL_MIN } from "@/lib/mapping-profile-quality";
...
function MappingProfileQualityPanel({ quality, job }: { quality: ReturnType<typeof analyseMappingProfile>; job: JobDescription }) {
  ...
  return <article className={`panel profile-quality ${quality.readyForReview ? "ready" : "attention"}`}><header><div><span className="section-kicker">ZM-13 · PROFILE QUALITY GATE</span><h3>Mutually exclusive, collectively exhaustive role profile</h3></div><b>{quality.readyForReview ? "Review ready" : "Action required"}</b></header><div className="mapping-diagnostics"><span><b>{quality.skillCount}</b><small>skills · target {ROLE_SKILL_MIN}–{ROLE_SKILL_MAX}</small></span><span><b>{coverage}%</b><small>exclusive evidence coverage</small></span><span><b>{quality.overlappingEvidence.length}</b><small>overlapping primary owners</small></span><span><b>{quality.averageInterval.lower}–{quality.averageInterval.upper}%</b><small>profile confidence interval</small></span></div><div className="coverage-track" aria-label={`${coverage}% exclusive evidence coverage`}><i style={{ width: `${coverage}%` }}/></div>{quality.findings.length ? <ul>{quality.findings.map((finding) => <li key={finding}>{finding}</li>)}</ul> : <p>All {job.evidenceSegments.filter((item) => ["responsibility", "outcome"].includes(item.normalizedType)).length} material responsibility and outcome statements have one primary skill owner, the profile contains {quality.skillCount} distinct skills and weights total 100%.</p>}<small className="helper-text">The confidence range is an evidence-adjusted operational interval for reviewer prioritisation, not a statistical population estimate.</small></article>;
}
