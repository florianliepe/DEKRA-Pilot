...
import { profileGuidance, proficiencyLevels, type Lifecycle, type Skill, type SkillDimension, type SkillWorkspace } from "@/lib/skill-schema";

type Tab = "overview" | "intake" | "library" | "taxonomy" | "profiles" | "review";
type SkillDraft = Pick<Skill, "name" | "description" | "groupId" | "dimension" | "kflaCompetencyId" | "observability" | "futureRelevance" | "status"> & { aliases: string };
...
const tabs: Array<{ id: Tab; label: string }> = [
  ...
  { id: "profiles", label: "Role profiles" }, { id: "review", label: "Review queue" },
];
const emptySkill = (groupId: string): SkillDraft => ({ name: "", description: "", groupId, dimension: "technical", kflaCompetencyId: "", aliases: "", observability: "", futureRelevance: "core", status: "draft" });
...
export function SkillDesigner({ workspaceSecret }: { workspaceSecret: string }) {
  ...
  useEffect(() => {
    ...
    void loadSkillWorkspace(workspaceSecret).then((payload) => {
      if (!current) return;
      if (payload.workspace?.skills && payload.workspace?.kfla) { setWorkspace(payload.workspace); setSync("live"); }
...
function Review({ workspace, mutate }: { workspace: SkillWorkspace; mutate: (update: (current: SkillWorkspace) => SkillWorkspace) => void }) {
  ...
  function decide(id: string, status: "accepted" | "rejected") { mutate((current) => ({ ...current, reviewQueue: current.reviewQueue.map((item) => item.id === id ? { ...item, status } : item), skills: status === "accepted" ? current.skills.map((skill) => skill.name === current.reviewQueue.find((item) => item.id === id)?.title ? { ...skill, status: "approved" as Lifecycle } : skill) : current.skills })); }
