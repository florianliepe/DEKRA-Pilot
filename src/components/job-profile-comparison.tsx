...
import { deriveRoleProfile } from "@/lib/derived-role-profile";
...
export function JobProfileComparison({ workspace, approvedWorkspace, selectedJobId, onOpenMapping, onOpenProfiles }: Props) {
  ...
  const derived = deriveRoleProfile(job, activeMappings, workspace, profile);
  ...
  return <div className="job-profile-comparison">
    ...
    <div className="job-profile-columns">
      ...
      <section className="panel"><header><span className="section-kicker">DERIVED SKILL PROFILE</span><h3>{derived.title}</h3><small>{derived.id} · derived from {job.id} · {profile ? `stored draft ${profile.status}` : "not yet stored"}</small></header><p>{derived.purpose || "No normalized role purpose is available."} The source job description remains unchanged.</p><div className="job-profile-skill-list">{selectedMappings.length ? selectedMappings.map((mapping) => <SkillLine key={mapping.id} mapping={mapping} workspace={workspace} inProfile={profileSkills.some((item) => item.skillId === mapping.skillId)} approved={approvedKeys.has(mapping.skillId)}/>) : <p>No active mappings yet. Run governed mapping from Jobs & mapping.</p>}</div><p className="job-profile-cap">{derived.skills.length}/10 core skills · {quality.technicalCount}/5 technical · {quality.behavioralCount}/5 behavioral · {quality.totalWeight}% weight · {quality.readyForReview ? "review ready" : "review blocked"}</p></section>
    </div>
    <section className="panel job-profile-diagnostics"><header><span className="section-kicker">RECONCILIATION & NEXT ACTION</span><h3>What needs a decision?</h3></header>{!profile && <p>No stored profile is linked to <code>{job.id}</code> yet. The derived view above remains available; accountable mapping review creates or synchronizes an in-review profile.</p>}{profileOnly.length > 0 && <p>Profile-only skills without an active job mapping: {profileOnly.map((item) => skillName(item.skillId)).join(", ")}.</p>}{mappingOnly.length > 0 && profile && <p>Mapped skills absent from the stored profile: {mappingOnly.map((item) => skillName(item.skillId)).join(", ")}.</p>}{quality.findings.map((item) => <p key={item}>{item}</p>)}{!quality.findings.length && profile && !profileOnly.length && !mappingOnly.length && <p>The active mappings and stored profile are aligned; human approval still controls publication.</p>}<p>Approved baseline: {approvedMappings.length} mapping(s). Proposed changes remain working state until accountable review and release.</p><div className="record-actions"><button className="button primary" onClick={() => onOpenMapping(job.id)}>Open Jobs & mapping</button><button className="button secondary" onClick={onOpenProfiles}>Open Role profiles</button></div></section>
  </div>;
}
