...
export function MultilingualLabelWorkbench({ workspace, mutate, onMessage, onError }: Props) {
  ...
  const [statusChange, setStatusChange] = useState<{ value: LocalizedConceptLabel; status: "draft" | "archived" } | null>(null);
  ...
  function toggle(value: LocalizedConceptLabel) {
    ...
    setStatusChange({ value, status });
  }

  function applyStatus(actor: string, reason: string) {
    if (!statusChange) return;
    try {
      mutate((current) => setLocalizedConceptLabelStatus(current, statusChange.value.id, statusChange.status, actor, reason));
      onMessage(`${statusChange.value.label} ${statusChange.status === "archived" ? "archived" : "restored to draft"}.`);
  ...
  return <section className="panel multilingual-workbench">
    ...
    {statusChange && <LabelLifecycleEditor value={statusChange.value} status={statusChange.status} onClose={() => setStatusChange(null)} onSave={applyStatus}/>}
  </section>;
}
...
function LabelLifecycleEditor({ value, status, onClose, onSave }: { value: LocalizedConceptLabel; status: "draft" | "archived"; onClose: () => void; onSave: (actor: string, reason: string) => void }) {
  const [actor, setActor] = useState(""); const [reason, setReason] = useState("");
  return <div className="modal-backdrop"><form className="modal-card" onSubmit={(event) => { event.preventDefault(); onSave(actor, reason); }}><header><div><span className="section-kicker">ACCOUNTABLE LABEL LIFECYCLE</span><h3>{status === "archived" ? "Archive" : "Restore"} {value.label}</h3></div><button type="button" onClick={onClose}><Icons.close/></button></header><p>The canonical concept remains unchanged and the translation history is preserved.</p><label><span>Accountable actor</span><input required value={actor} onChange={(event) => setActor(event.target.value)}/></label><label><span>Governance reason</span><textarea required value={reason} onChange={(event) => setReason(event.target.value)}/></label><footer><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary">Apply label lifecycle</button></footer></form></div>;
}
