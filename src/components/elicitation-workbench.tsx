...
export function ElicitationWorkbench({ workspace, secret, mutate, onWorkspace, onMessage, onError }: Props) {
  ...
  function submit() {
    ...
    mutate((current) => recordGovernedVersion({ ...current, elicitationSessions: current.elicitationSessions.map((item) => item.id === draft.id ? { ...draft, status: "submitted", updatedAt: now } : item), reviewQueue: [{ id: `REV-${draft.id}`, title: draft.fields.capability || draft.title, type: "new_skill", summary: "Guided elicitation package submitted for syntax, granularity and taxonomy review.", confidence: completion, evidence: `${draft.fields.activities} · ${draft.fields.outcomes}`, explanation: "The agent may validate and rewrite, but an accountable human must approve the canonical skill.", rulesVersion: current.framework.rulesVersion, frameworkVersion: current.framework.version, status: "pending", entityId: draft.id, payload: { sessionId: draft.id, requestedBy: actor.trim(), requestReason: reason.trim() } }, ...current.reviewQueue] }, "elicitation_session", draft.id, "elicitation.submitted", actor.trim(), { ...draft, governanceReason: reason.trim() } as unknown as Record<string, unknown>));
  ...
  return <div className="elicitation-layout">
    ...
    <section className="panel elicitation-main"><header><div><span className="section-kicker">STEP {draft.currentStep + 1} OF {steps.length}</span><h3>{step.label}</h3><p>{step.guidance}</p></div><div className="elicitation-progress"><b>{completion}%</b><span><i style={{ width: `${completion}%` }}/></span></div></header><div className="elicitation-step"><StepField session={draft} step={step} workspace={workspace} onChange={setField}/></div><div className="form-row"><label><span>Accountable actor</span><input value={actor} onChange={(event) => setActor(event.target.value)} placeholder="Named capability owner"/></label><label><span>Governance reason</span><textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Why is this evidence package being saved or submitted?"/></label></div><footer><button className="button secondary" disabled={draft.currentStep === 0} onClick={() => setDraft((current) => ({ ...current, currentStep: current.currentStep - 1 }))}>Previous</button><button className="button secondary" disabled={!actor.trim() || !reason.trim()} onClick={save}>Save draft</button><button className="button secondary" disabled={busy || !actor.trim() || !reason.trim()} onClick={() => void assist("rewrite")}><Icons.spark/>AI-assisted rewrite</button>{draft.currentStep < steps.length - 1 ? <button className="button primary" onClick={() => setDraft((current) => ({ ...current, currentStep: current.currentStep + 1 }))}>Next</button> : <button className="button primary" disabled={!actor.trim() || !reason.trim()} onClick={submit}>Submit for review</button>}</footer></section>
