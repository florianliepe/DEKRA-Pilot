...
export const validationRules: ValidationRule[] = [
  ...
  ["MAPPING-SCORE-VERSION-001", "Active score model", "Mappings must use the active score model before approval.", "error", "scoreVersion", "Re-evaluate against the active score model.", true],
  ["MAPPING-LINEAGE-001", "Versioned mapping lineage", "Agent mappings must retain framework, rules, prompt, score and evaluation-dataset versions.", "error", "promptVersion", "Re-evaluate with active governed versions.", true],
  ["MAPPING-FEEDBACK-001", "Feedback accountability", "Mapping feedback must retain an existing mapping, reviewer, reason and valid evidence completeness.", "error", "mappingId", "Complete the accountable feedback record.", true],
...
export const bootstrapSkillWorkspace: SkillWorkspace = {
  ...
  profiles: [{
    ...
    excludedLinks: [
      { skillId: "SK-ST", reason: "The taxonomy-design concept is not approved and has no direct role evidence.", status: "unapproved_skill" },
      { skillId: "SK-CU", reason: "Trait evidence is insufficient for inclusion in the governed job profile.", status: "coverage_gap" },
    ],
    agentRunId: "RUN-001",
  }],
  ...
  jobClarifications: [{ id: "CLAR-JD-DATA", jobDescriptionId: "JD-DATA", status: "in_progress", currentQuestion: 1, startedAt: "2026-08-06T09:55:00.000Z", updatedAt: "2026-08-06T10:00:00.000Z", idempotencyKey: "clarification-jd-data-v1", sessionVersion: 1, sufficiencyScore: 82, sufficiencyThreshold: 80, canMap: true, stopReason: "user_requested_more", contradictions: [], questions: [
    { id: "CLAR-JD-DATA-OUTCOME", dimension: "outcomes", question: "Which management decision must the weekly reporting enable?", rationale: "The source names management insight but not the decision it changes.", answer: "Prioritise corrective commercial action across regions.", evidenceRecordId: "EVD-JD-DATA-CLAR-001", status: "answered" },
    { id: "CLAR-JD-DATA-AUTONOMY", dimension: "autonomy", question: "Which reporting decisions can the analyst make independently?", rationale: "Autonomy is needed to calibrate proficiency.", affectedMappingDimensions: ["proficiencyCompatibility", "responsibilityCoverage", "contradictionPenalty"], sourceExcerpts: [{ evidenceId: "JD-DATA-SEG-02", label: "Global Reporting Analyst JD · responsibility 2", excerpt: "Analyse performance drivers and explain material movements to senior stakeholders." }], gapType: "weak", priority: "medium", blocking: false, status: "open" },
    { id: "CLAR-JD-DATA-COMPLEXITY", dimension: "complexity", question: "Describe a case where operational and financial indicators contradicted each other.", rationale: "A critical incident distinguishes routine reporting from complexity management.", status: "open" },
  ] }],
  ...
  reviewQueue: [
    ...
    { id: "REV-003", entityId: "ROLE-DATA", title: "Review Global Reporting Analyst profile", type: "profile", summary: "Agent-composed role profile grounded in the selected job description and approved taxonomy.", confidence: 86, evidence: "Global Reporting Analyst JD · responsibilities and outcomes", explanation: "The proposed profile remains in review until an accountable human accepts, defers or rejects it.", frameworkVersion: "3.1.0", rulesVersion: "rules-3.1.0", status: "pending", payload: { operation: "profile_review", jobDescriptionId: "JD-DATA", requestedBy: "skill-design-agent" } },
    { id: "REV-004", entityId: "MAP-MC", title: "Review Managing Complexity mapping", type: "mapping", summary: "Explaining material performance movements requires structuring complex information.", confidence: 83, evidence: "Analyse performance drivers and explain material movements.", explanation: "The evidence-grounded AI mapping remains proposed until an accountable human accepts, defers or rejects it.", frameworkVersion: "3.1.0", rulesVersion: "rules-3.1.0", status: "pending", payload: { operation: "mapping_review", jobDescriptionId: "JD-DATA", skillId: "SK-MC", requestedBy: "skill-design-agent" } },
  ],
  kflaFactors,
  kflaClusters,
  ...
  jobDescriptions: [{
    ...
    outcomes: ["Decision-ready management insight", "Consistent reporting definitions", "Improved data quality"],
    activities: ["Build dashboards", "Analyse performance drivers", "Explain material movements", "Maintain reporting definitions"],
    tools: ["Power BI"], qualifications: [], context: ["Global weekly revenue reporting"], constraints: ["Consistent definitions", "Trusted data quality"],
    sourceFiles: [{ name: "global-reporting-analyst.md", mediaType: "text/markdown", size: 284, contentHash: "synthetic-fixture-v1" }],
    intakeFindings: [], intakeIdempotencyKey: "job-intake-jd-data-v1",
    evidenceSegments: [
      { id: "SEG-JD-DATA-01", sourceId: "SRC-JD-DATA", sourceName: "global-reporting-analyst.md", section: "Responsibilities", location: "responsibility 1", quotation: "Build interactive dashboards to report weekly revenue metrics.", normalizedType: "responsibility", normalizedValue: "Build interactive dashboards for weekly revenue reporting.", confidence: 98 },
      { id: "SEG-JD-DATA-02", sourceId: "SRC-JD-DATA", sourceName: "global-reporting-analyst.md", section: "Responsibilities", location: "responsibility 2", quotation: "Analyse performance drivers and explain material movements to senior stakeholders.", normalizedType: "responsibility", normalizedValue: "Analyse performance drivers and material movements.", confidence: 96 },
      { id: "SEG-JD-DATA-03", sourceId: "SRC-JD-DATA", sourceName: "global-reporting-analyst.md", section: "Outcomes", location: "derived outcome 1", quotation: "Turn operational and financial data into trusted management insight.", normalizedType: "outcome", normalizedValue: "Decision-ready management insight", confidence: 91 },
      { id: "SEG-JD-DATA-04", sourceId: "SRC-JD-DATA", sourceName: "global-reporting-analyst.md", section: "Tools", location: "tool reference 1", quotation: "Build interactive dashboards", normalizedType: "tool", normalizedValue: "Power BI", confidence: 72 },
    ],
    status: "mapped", version: 1, updatedAt: "2026-08-06T10:00:00.000Z",
  }],
  mappings: [
    { id: "MAP-DV", jobDescriptionId: "JD-DATA", skillId: "SK-DV", targetLevel: 3, weight: 35, critical: true, relevance: 93, confidence: 92, rationale: "Dashboard creation and executive reporting require advanced visualisation capability.", evidence: ["Build interactive dashboards to report weekly revenue metrics."], evidenceRefs: ["SEG-JD-DATA-01"], strategicVectorIds: ["VEC-AI"], toolIds: ["TOOL-POWERBI"], scoreBreakdown: score({ semanticRelevance: 98, directEvidenceStrength: 98, responsibilityCoverage: 95, outcomeRelevance: 94, taxonomyCompatibility: 96, granularityCompatibility: 94, kflaCompatibility: 72, controlledToolRelevance: 98, proficiencyCompatibility: 92, approvedMappingSimilarity: 90 }), scoreVersion: "mapping-2.0.0", frameworkVersion: "3.1.0", rulesVersion: "rules-3.1.0", promptVersion: "skill-agent-2.0.0", evaluationDatasetVersion: "DEKRA-MAPPING-GOLDEN-001@2.0.0", source: "agent", status: "approved", agentRunId: "RUN-001" },
    { id: "MAP-MC", jobDescriptionId: "JD-DATA", skillId: "SK-MC", targetLevel: 2, weight: 20, critical: true, relevance: 84, confidence: 83, rationale: "Explaining material performance movements requires structuring complex information.", evidence: ["Analyse performance drivers and explain material movements."], evidenceRefs: ["SEG-JD-DATA-02"], kflaCompetencyIds: ["KFLA-08"], strategicVectorIds: ["VEC-LEAD"], toolIds: [], scoreBreakdown: score({ semanticRelevance: 87, directEvidenceStrength: 86, responsibilityCoverage: 84, outcomeRelevance: 82, taxonomyCompatibility: 88, granularityCompatibility: 85, kflaCompatibility: 92, controlledToolRelevance: 40, proficiencyCompatibility: 78, approvedMappingSimilarity: 80 }), scoreVersion: "mapping-2.0.0", frameworkVersion: "3.1.0", rulesVersion: "rules-3.1.0", promptVersion: "skill-agent-2.0.0", evaluationDatasetVersion: "DEKRA-MAPPING-GOLDEN-001@2.0.0", source: "agent", status: "proposed", agentRunId: "RUN-001" },
  ],
  mappingOmissions: [{ id: "OMIT-JD-DATA-MC-CORE", jobDescriptionId: "JD-DATA", skillId: "SK-MC", reason: "Managing Complexity was omitted from the critical dashboard-delivery core because its evidence supports a lower-weight, supporting mapping only.", evidenceRefs: ["SEG-JD-DATA-02", "SEG-JD-DATA-03"], score: 52, status: "explained", agentRunId: "RUN-001" }],
  strategicVectors: [
  ...
  agentRuns: [{ id: "RUN-001", mode: "job_mapping", status: "needs_review", jobDescriptionId: "JD-DATA", startedAt: "2026-08-06T09:58:00.000Z", completedAt: "2026-08-06T10:00:00.000Z", model: "claude-sonnet-5", tools: ["read_catalog", "find_duplicates", "map_job_skills", "propose_profile"], trace: [{ step: "Evidence audit", result: "3 responsibility statements retained" }, { step: "Taxonomy match", result: "2 approved skills matched; 1 proposal requires review" }], idempotencyKey: "map-jd-data-v1", attempt: 1 }],
  tools: [
  ...
  evidenceRecords: [
    ...
    { id: "EVD-JD-DATA-CLAR-001", sourceId: "SRC-JD-DATA", summary: "The weekly reporting enables prioritisation of corrective commercial action across regions.", location: "clarification CLAR-JD-DATA-OUTCOME", dataClassification: "internal", supportedEntityIds: ["JD-DATA", "CLAR-JD-DATA"], confidence: 90, status: "in_review" },
  ],
  localizedLabels: [
