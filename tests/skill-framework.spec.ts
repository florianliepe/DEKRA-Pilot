...
test("calculates the transparent thirteen-part weighted score and penalties", () => {
  ...
  const contributions = mappingScoreContributions(base, bootstrapSkillWorkspace.framework.mappingWeights);
  expect(contributions).toHaveLength(13);
  expect(contributions.find((item) => item.key === "duplicatePenalty")).toMatchObject({ direction: "penalty", weight: 4 });
});

test("passes the versioned golden mapping and abstention evaluation", () => {
  const dataset = JSON.parse(readFileSync(join(process.cwd(), "data/evaluation/mapping-golden-baseline.json"), "utf8")) as MappingEvaluationDataset;
  const report = evaluateMappingDataset(dataset, bootstrapSkillWorkspace);
  expect(report.versionAligned).toBe(true);
  expect(report.results.length).toBeGreaterThanOrEqual(10);
  expect(report).toMatchObject({ failed: 0, passRate: 100, mappingAccuracy: 100, abstentionAccuracy: 100 });
});

test("blocks stale score versions and unexplained score overrides", () => {
  const workspace = structuredClone(bootstrapSkillWorkspace);
  workspace.mappings[0].scoreVersion = "mapping-legacy";
  workspace.mappings[0].relevance = 1;
  workspace.mappings[0].source = "manual";
  const findings = validateWorkspace(workspace);
  expect(findings).toContainEqual(expect.objectContaining({ ruleId: "MAPPING-SCORE-VERSION-001", affectedField: "scoreVersion", blocking: true }));
  expect(findings).toContainEqual(expect.objectContaining({ ruleId: "MAPPING-OVERRIDE-001", affectedField: "overrideReason", blocking: true }));
});
...
test("records accountable mapping feedback without changing approval status", () => {
  ...
  expect(() => recordMappingFeedback(workspace, { mappingId: "MAP-DV", decision: "confirmed", reviewer: "", reviewerRole: "taxonomy_steward", reason: "Evidence verified." })).toThrow(/reviewer/i);
  const reviewed = recordMappingFeedback(workspace, { mappingId: "MAP-DV", decision: "adjusted", reviewer: "Mapping Steward", reviewerRole: "taxonomy_steward", reason: "Direct evidence is strong, but confidence is reduced pending a second role sample.", confidenceAfter: 86 });
  ...
  expect(mappingCalibrationSummary(reviewed)).toMatchObject({ sampleSize: 1, calibratableSize: 1, predicted: 92, observed: 0, calibrationGap: -92, evidenceCompleteness: 100 });
